...
//    Overloading overloadingChild=new OverloadingChild();
//    overloadingChild.notInParent();

    }

    private static void stringWithNull() {
        String s1="ABC"+null;
        String s2=null+"ABC";
    ...
    private static void enumTesting() {
        EnumTest.printAll();
        EnumTest.EAST.printDetails();
    }

    private static void arraysRuntimeTypr() {
        int []intArray=new int[]{1,2,4};
        System.out.println(intArray.getClass().getName());

        Integer []integerArray=new Integer[]{1,2,4};
        System.out.println(integerArray.getClass().getName());

        String []stringArray=new String[]{"ab","ac","ad"};
        System.out.println(stringArray.getClass().getName());

        Employee[] employeesArray=new Employee[1];
        System.out.println(employeesArray.getClass().getName());

        int [][]int2DArray=new int[][]{{1,2},{3,4},{5,6}};
        System.out.println(int2DArray.getClass().getName());


        int[] intCopy=new int[3];
        System.arraycopy(intArray,0,intCopy,0,3);
    }
